...
class MaturityProfile:
    ...
    def total(self) -> float:
        ...
        return (
            self.within_1_year + 
            self.between_1_and_3_years + 
            self.between_3_and_5_years + 
            self.beyond_5_years
        )
...
class DebtStructureAnalyzer:
    ...
    def extract_covenants_from_text(self, text: str) -> List[Covenant]:
        ...
        for covenant_type, pattern in self.COVENANT_PATTERNS.items():
            matches = re.finditer(
                f"({pattern})[^.]*?(>=?|<=?|at\s*least|not\s*exceed)[^.]*?([\d.]+)",
                text,
                re.IGNORECASE
            )
    ...
    def assess_debt_risk(self, result: DebtStructure) -> Dict[str, str]:
        ...
        # Refinancing risk
        maturity_risk = result.get_maturity_risk()
        risks["refinancing"] = f"{maturity_risk} - {result.maturity_profile.within_1_year:.1f} Cr maturing within 1 year"
        
        # Concentration risk
        conc_risk = result.get_concentration_risk()
        if result.top_lender:
            risks["concentration"] = f"{conc_risk} - Top lender {result.top_lender} has {result.top_lender_exposure:.1f} Cr exposure"
        else:
        ...
        if result.interest_coverage_ratio:
            if result.interest_coverage_ratio < 1.5:
                risks["coverage"] = f"HIGH - Interest coverage ratio {result.interest_coverage_ratio:.2f}x is below 1.5x"
            elif result.interest_coverage_ratio < 3.0:
                risks["coverage"] = f"MEDIUM - Interest coverage ratio {result.interest_coverage_ratio:.2f}x is below 3x"
            else:
