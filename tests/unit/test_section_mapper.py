...
def sample_section_tree():
    ...
    section1 = Section(
        section_id="sect_001", section_type=SectionType.SUMMARY, title="Summary", level=1, start_page=1, end_page=5
    )
    section2 = Section(
        section_id="sect_002",
        section_type=SectionType.RISK_FACTORS,
...
class TestSection:
    ...
    def test_section_defaults(self):
        ...
        section = Section(
            section_id="test", section_type=SectionType.OTHER, title="Test Section", level=1, start_page=1, end_page=5
        )
...
class TestSectionTree:
    ...
    def test_get_section_by_title_exact_match(self, sample_section_tree):
        """Test retrieving section by title with fuzzy=False (exact match)."""
        # Exact match should work
        section = sample_section_tree.get_section_by_title("Our Business", fuzzy=False)
        assert section is not None
        assert section.section_type == SectionType.BUSINESS

        # Partial match should fail with fuzzy=False
        section = sample_section_tree.get_section_by_title("Business", fuzzy=False)
...
class TestSectionMapper:
    ...
    def test_determine_header_level_major_section(self, section_mapper):
        ...
        level = section_mapper._determine_header_level(
            text="RISK FACTORS", font_size=18.0, baseline=12.0, has_numbering=False
        )
    ...
    def test_determine_header_level_subsection(self, section_mapper):
        ...
        level = section_mapper._determine_header_level(
            text="1.1 Internal Risks", font_size=14.0, baseline=12.0, has_numbering=True
        )
...
class TestEdgeCases:
    ...
    def test_very_short_and_long_lines_skipped(self, section_mapper):
        """Test that very short (<3 chars) and very long (>150 chars) lines are skipped."""
        page = Mock()
        page.page_num = 1
        # Include: very short line, valid header, very long line
        long_line = "A" * 200  # More than 150 chars
        page.text = f"AB\nRISK FACTORS\n{long_line}\nActual content here"
        page.word_count = 100
        page.has_tables = False
        page.font_sizes = [18.0, 12.0]
        ...
        # Should still find the valid RISK FACTORS header
        assert tree is not None
        risk_sections = tree.get_section_by_type(SectionType.RISK_FACTORS)
        assert len(risk_sections) == 1# =============================================================================
